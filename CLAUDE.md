...
- Cuando se considere importante agregar imágenes a un texto (para ilustrar un concepto, un proceso o un resultado esperado), buscarlas en la bibliografía disponible (`Bibliografia/`) o en internet, priorizando páginas de instituciones de referencia (organismos oficiales, universidades, agencias espaciales, proveedores de datos reconocidos, etc.), o generarlas con código propio (ver `graficos_raster.py` y "Trabajo con los notebooks" más abajo) cuando conviene ilustrar valores concretos sobre una grilla ráster sintética. Toda imagen debe llevar epígrafe definido, cerrado siempre con una oración de fuente en el formato `Fuente: <cita>.`: `Fuente: [@clave].` (formato APA 7ª edición, vía cita de `references.bib`) cuando la imagen proviene de o está adaptada de una fuente externa, o `Fuente: Elaboración propia.` cuando es una figura original del equipo docente (por ejemplo, generada con `graficos_raster.py`), sin cita externa. Por ejemplo: *"...(la celda gris clara representa NoData, que se propaga al resultado). Fuente: Elaboración propia."*
- Toda imagen debe además mencionarse explícitamente en el cuerpo del texto que la acompaña (con una referencia cruzada `@fig-id` de Quarto, o al menos remitiendo a ella en la prosa cercana) — no alcanza con insertarla como figura flotante sin nombrarla en el desarrollo.
...
- `Clases/` — Material de clase, dividido en tres subcarpetas:
  ...
  - `Actividades prácticas/` — parciales, rúbricas y TPs de cohortes previas (`Evaluacion_Parcial_*`, `EvaluacionParcial_II.docx`, `RubicaEvaluacion_Parcial_I.docx`, `Copia de Parcial III.docx`, `TPII.pptx`), el notebook de Colab `Clase_X_Análisis_espacio_temporal_Cubo_de_Datos.ipynb` sobre cubos de datos geoespaciales, y los notebooks `Ejercícios_operadores_booleanos_raster.ipynb` (ejercicios de operadores booleanos/monocriterio sobre ráster) y `Figuras_Cuadernillo.ipynb` (genera figuras usadas en los textos del Cuadernillo) — agrupados aparte de las presentaciones teóricas por ser instrumentos de práctica/evaluación.
...
- `Cuestionarios/` — los cuestionarios autoevaluativos de la instancia conceptual (ver "Actividades autoevaluativas" más arriba), uno por texto del Cuadernillo, en `nuevos/` como formato `.gift` (texto plano importable a Moodle); sin subcarpeta `antiguos/` porque no hay una versión previa a actualizar. Cada `.gift` puede tener su propia carpeta `imagenes/` con figuras generadas para preguntas de opción múltiple (ver "Trabajo con los notebooks" más abajo) — a diferencia de las imágenes del Cuadernillo, estas quedan como placeholder de ruta relativa en el `<img src="...">` del `.gift`, porque GIFT es texto plano y no empaqueta archivos: hay que subirlas a la pregunta desde el editor de Moodle después de importar.
...
- `Juego/` — el juego de práctica de álgebra de mapas en Streamlit ("Álgebra de mapas: el desafío"): preguntas generadas proceduralmente en 5 niveles de dificultad crecientes (alineados al orden del Cuadernillo), dos modos por ronda ("indicá la operación realizada" / "indicá el resultado"), 3 vidas, registro de nombre y ranking de los 5 mejores puntajes (SQLite local a la app). Reutiliza `plot_raster()` de `graficos_raster.py` (raíz del repo) sin duplicarlo ni modificarlo. Ver `Juego/README.md` para correrlo local o desplegarlo en Streamlit Community Cloud, incluida la advertencia sobre la persistencia del ranking.
- Raíz — `Reflexiones SIG 3 2025.docx` (ver sección de aprendizajes arriba). También `_quarto.yml`, `references.bib` y `apa.csl` — setup compartido de producción de contenido (ver "Producción de textos y presentaciones con Quarto" más abajo) —, `diagnostico_textos.md`, la revisión tema por tema del Cuadernillo y la planificación, y `graficos_raster.py`, módulo Python compartido con `plot_raster()` (grilla de ráster con el valor de cada celda escrito encima) para las figuras didácticas de `Figuras_Cuadernillo.ipynb`, reutilizado también por `Juego/` (ver "Trabajo con los notebooks" abajo).

## Trabajo con los notebooks

Los notebooks de `Clases/Actividades prácticas/` usan `numpy`, `matplotlib`, `scipy` (`convolve2d`) y `rasterio` (instalado dentro del notebook con `!pip install rasterio --quiet`) para armar grillas ráster sintéticas (como arrays enmascarados de `numpy`) y visualizar operaciones ráster (superposición booleana, filtros focales) con fines didácticos — son ilustrativos, no una librería ni un pipeline. `Figuras_Cuadernillo.ipynb` existe específicamente para (re)generar figuras embebidas en los textos del Cuadernillo, así que las figuras regeneradas deben mantenerse visualmente consistentes con cómo se las referencia en esos capítulos; para eso importa `plot_raster()` de `graficos_raster.py` (raíz del repo), el módulo compartido de graficado que centraliza ese estilo (grilla + valor de celda) en vez de duplicar el código en cada celda, el mismo que reutiliza `Juego/` (el juego de práctica de álgebra de mapas en Streamlit). El mismo notebook también genera, en una sección aparte, figuras "misterio" (sin título que revele la operación aplicada, con valores distintos a los ya usados en el Cuadernillo) para las preguntas de opción múltiple con imagen de `Cuestionarios/nuevos/`, guardándolas en la carpeta `imagenes/` de ese cuestionario en vez de la del Cuadernillo. El notebook `Clase_X_...` está escrito para Google Colab (monta Google Drive para acceder a una carpeta de datos compartida), no para ejecución local.

`plot_raster()` sigue algunas convenciones fijas para que las figuras nuevas se vean consistentes con las ya hechas: las celdas NoData se marcan con relleno gris, la etiqueta `SD` (sin dato) y un borde rojo fino, deliberadamente más delgado que el recuadro de `highlight` (usado para "seguir una celda puntual" entre paneles), para no confundir ambos recursos si coinciden en una misma celda. Para capas categóricas, activar `categorical=True` y elegir el `cmap` según el tipo de categoría: uno secuencial (por ejemplo `Oranges`) para categorías ordinales, con un orden real entre sus clases (pendiente clasificada en baja/media/alta); uno cualitativo (por ejemplo `Set2`, `tab10`) para categorías nominales, sin orden entre sus clases (tipos de uso del suelo), para no sugerir con el color una gradación que no existe.
